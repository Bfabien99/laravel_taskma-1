...
<section class="flex m-auto max-w-screen-lg grow p-2">
    <form action="{{route('tasks.store')}}" method="post" class="mx-auto p-4 bg-white rounded-sm shadow-sm flex flex-col gap-2 w-full max-w-[500px]">
        ...
        <div class="flex flex-col">
            ...
            <input type="text" class="border p-2 rounded-sm" name="name" value="{{old('name')}}" placeholder="Enter task name">
            @error('name')
        ...
        <div class="flex flex-col">
            <label for="" class="font-medium capitalize">Project ID : {{request()->input('project_id')}}</label>
        ...
        @endif
        <button class="bg-gray-900 hover:bg-black text-white p-2 rounded-md font-medium">Create Task</button>
    </form>
</section>
@endsection